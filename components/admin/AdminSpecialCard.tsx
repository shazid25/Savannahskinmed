...
import { deleteSpecialCardAction } from '@/app/admin/dashboard/content/specials/actions';
...
export default function AdminSpecialCard({ card }: SpecialCardProps) {
  ...
  return (
    ...
    >
      {/* Visual Card Header — looks like public OfferCard */}
      ...
      {expanded && (
        <div className="border-t border-navy/10 bg-cream/30 p-5">
          <input type="hidden" name="cardIds" value={card.id} form="specials-save" />
          <div className="space-y-4">
            <CloudinaryUpload
              ...
              label="Card Image"
            />
            <input type="hidden" name={`card-image-${card.id}`} value={image} form="specials-save" />

            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-[12px] text-muted">Image Alt Text</label>
                <input
                  name={`card-imageAlt-${card.id}`}
              ...
              <div className="space-y-3">
                ...
                {card.tiers.map((tier) => (
                  <div key={tier.id} className="grid items-end gap-2 sm:grid-cols-[2fr_2fr_auto]">
                    <input type="hidden" name="tierIds" value={tier.id} form="specials-save" />
                    <div>
